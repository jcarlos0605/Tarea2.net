...
using tarea2.Migrations.SeedData;
...
namespace tarea2.Tests
{
    public abstract class tarea2TestBase : AbpIntegratedTestBase<tarea2TestModule>
    {
        ...
        protected void UsingDbContext(Action<tarea2DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<tarea2DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<tarea2DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<tarea2DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<tarea2DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<tarea2DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<tarea2DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<tarea2DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<tarea2DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<tarea2DbContext, Task<T>> func)
        {
            T result;
